...
python -u /home/jmhong20/regime-based-portfolio/src/train_test.py --window_length $window_length --window_period $window_period --seed $seed --feature_type $feature_type --activation $activation --actor_predictor $actor_predictor --critic_predictor $critic_predictor --rebalancing $rebalancing --buy_fee $buy_fee --sell_fee $sell_fee --time_cost $time_cost --mdd_alpha $mdd_alpha --beta $beta --action_bound $action_bound --random_start $random_start --episode $episode --max_step $max_step --target_date $target_date --train_period $train_period --name $name --file_path $file_path
